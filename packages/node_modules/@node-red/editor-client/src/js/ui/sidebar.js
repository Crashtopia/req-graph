...
RED.sidebar = (function () {
    ...
    function init() {
        ...
        RED.sidebar.models.init();

        RED.sidebar.show("models");
        // hide info bar at start if screen rather narrow...
        if ($("#red-ui-editor").width() < 600) { RED.menu.setSelected("menu-item-sidebar", false); }
        RED.events.on("editor:open", function () {
            RED.sidebar.toggleSidebar(true);
            RED.sidebar.show("models");

            var node = RED.editor.getEditStack()[0];

            setTimeout(function () {
                var select = $("#node-input-model");
                select.empty();
                if (RED.sidebar.models.getKey("gemini")) {
                    select.append($("<option>").val("gemini-2.5-flash").text("Gemini 2.5 Flash"));
                }
                if (RED.sidebar.models.getKey("mistral")) {
                    select.append($("<option>").val("mistral-large-latest").text("Mistral Large"));
                }

                if (node.model) {
                    select.val(node.model);

                }
                else {
                    // pick first available option as default (gemini → mistral → nothing)
...
function updateAllNodeModels(deletedModelKey) {
    var fallback = null;
    if (deletedModelKey !== "gemini" && RED.sidebar.models.getKey("gemini")) {
        fallback = "gemini-2.5-flash";
    } else if (deletedModelKey !== "mistral" && RED.sidebar.models.getKey("mistral")) {
        fallback = "mistral-large-latest";
    }

    RED.nodes.eachNode(function (node) {
        if (node.model && node.model.startsWith(deletedModelKey)) {
            node.model = fallback || "";
            node.changed = true;
        }
    });
...
RED.sidebar.models = (function () {
    var content;

    function createModelSection(modelName, modelKey) {
        ...
        // Warning badge next to title
        var warning = document.createElement("span");
        warning.textContent = " ⚠ API key not set";
        ...
        input.type = "password";
        input.placeholder = "Enter " + modelName + " API key...";
        input.style.width = "100%";
        input.style.marginBottom = "4px";
        input.className = "red-ui-typedInput";
        ...
        btn.onclick = function () {
            ...
            if (key) {
                RED.settings.set("models." + modelKey + ".apiKey", key);
                RED.notify(modelName + " API key saved.", "success");
                warning.style.display = "none";
            } else {
                warning.style.display = "inline";
                RED.notify(modelName + " API key cannot be empty.", "warning");
            }
        ...
        deleteBtn.className = "red-ui-button";
        deleteBtn.style.marginLeft = "4px";
        deleteBtn.style.color = "#e74c3c";
        deleteBtn.onclick = function () {
            RED.settings.set("models." + modelKey + ".apiKey", "");
            input.value = "";
            warning.style.display = "inline";
            RED.notify(modelName + " API key deleted.", "success");
            $("#node-dialog-cancel").trigger("click");
            updateAllNodeModels(modelKey);
        };
        var savedKey = RED.settings.get("models." + modelKey + ".apiKey");
        if (savedKey) {
            input.value = savedKey;
            warning.style.display = "none";
        }

        // Also hide warning as user types
    ...
    function init() {
        ...
        content.id = "red-ui-sidebar-models";
        content.style.height = "100%";
        content.style.overflowY = "auto";
        content.style.padding = "8px";

        content.appendChild(createModelSection("Gemini", "gemini"));
        content.appendChild(createModelSection("Mistral", "mistral"));
    ...
    return {
        ...
        getKey: function (model) {
            return RED.settings.get("models." + model + ".apiKey") || null;
        }
    };
})();