...
module.exports = {
    ...
    executeWorkflow: function (req, res) {
        var opts = {
            user: req.user,
            flows: req.body,
        ...
        runtimeAPI.flows.executeWorkflow(opts).then(function (result) {
            res.json(result);
        }).catch(function (err) {
            apiUtils.rejectHandler(req, res, err);
        });
    }, resumeWorkflow: function (req, res) {
        var opts = {
            user: req.user,
            flows: req.body,
            apiKeys: req.body.apiKeys,
            humanInput: req.body.humanInput,
            previousConversation: req.body.previousConversation,
            resumeFromNodeId: req.body.resumeFromNodeId,
            previousLog: req.body.previousLog,  // <-- add this
            req: apiUtils.getRequestLogObject(req)
        }
        runtimeAPI.flows.resumeWorkflow(opts).then(function (result) {
            res.json(result);
        }).catch(function (err) {
            apiUtils.rejectHandler(req, res, err);
        });
