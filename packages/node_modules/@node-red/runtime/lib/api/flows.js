...
function detectLoop(flows) {
    ...
    function isAllowedLoopNode(nodeId) {
        ...
        return node && (node.type === "HITL" || node.type === "Judge");
    }
    ...
    executeWorkflow: async function (opts) {
        ...
        try {
            ...
            if (inputNodes.length !== 1)
                return {
                    status: "error",
                    message: "Wrong amount of input nodes, one is required.",
                };

            if (detectLoop(opts.flows.flows)) {
                return {
                    status: "error",
                    message: "Flow contains a loop without HITL or judge agent to escape it. Execution aborted."
                };
            }
            ...
            const containsHITL = opts.flows.flows.filter(n => n.type === "HITL").length > 0;

            const llm = opts.apiKeys.gemini ? new ChatGoogleGenerativeAI({
                apiKey: opts.apiKeys.gemini,
                model: "gemini-2.5-flash-lite",
                maxRetries: 0
            }) : null;

            const llmMistral = opts.apiKeys.mistral ? new ChatMistralAI({
                apiKey: opts.apiKeys.mistral,
            ...
            const nodeMap = {};
            const graph = {};
            opts.flows.flows.forEach(function (node) {
                nodeMap[node.id] = node;
                graph[node.id] = [];
                if (node.wires) {
                    node.wires.forEach(function (port) {
            ...
            // Get LLM for a node
            function getLLM(node) {
                const model = node.model ||
                    (opts.apiKeys.gemini ? "gemini-2.5-flash-lite" : null) ||
                    (opts.apiKeys.mistral ? "mistral-large-latest" : null);
                if (!model) throw new Error("No API keys set.");
                if (model.startsWith("gemini")) {
                    if (!llm) throw new Error("Gemini API key not set.");
                    return llm;
                }
                if (model.startsWith("mistral")) {
                    if (!llmMistral) throw new Error("Mistral API key not set.");
                    return llmMistral;
                }
                throw new Error("Unknown model: " + model);
            }

            // Invoke LLM for a node
            async function invokeNode(node, input, propagatedInput) {
                const stepStart = Date.now();
                const systemPrompt = (node.seedingPrompt || "") + (propagatedInput ? "\nThis is the overall goal: " + propagatedInput : "");
                const model = getLLM(node);
                ...
                log.steps.push({
                    ...
                    input: input,
                    inputPropagation: propagatedInput !== null,
            ...
            // Find start node (Text Input)
            let propagatedInput = null;
            ...
            if (startNode.propagate) {
                propagatedInput = startNode.text;
            }
            ...
            // Traverse graph and invoke LLMs
            const conversation = [];
            let currentOutput = startNode.text || "";
            let currentId = graph[startNode.id][0];

            while (currentId) {
                const node = nodeMap[currentId];
                if (!node) { runtime.log.info("Node not found: " + currentId); break; }

                if (!node.seedingPrompt) {
                    runtime.log.info("Skipping node (no seedingPrompt): " + node.name);
                    currentId = graph[currentId][0];
                    continue;
                }

                currentOutput = await invokeNode(node, currentOutput, propagatedInput);
