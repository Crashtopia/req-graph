 ...
 */
    ...
    executeWorkflow: async function (opts) {
        ...
        try {
            ...
            const llmMistral = new ChatMistralAI({
                apiKey: opts.apiKeys.mistral,
                model: "mistral-large-latest",
                maxRetries: 0
            });
            const responseMistral = await llmMistral.invoke([
                new SystemMessage("You are a mistral agent"),
                new HumanMessage("Say something cool")
            ]);

            var result = await runtime.flows.setFlows(
                opts.flows.flows,
                opts.flows.credentials,
                "full",
                null,
                null,
                opts.user
            ...
            return {
                ...
                HITL: true,
                conversation: [
                    { agent: "Gemni", message: response.content },
                    { agent: "Mistral", message: responseMistral.content }
                ]
            };
        } catch (err) {
            runtime.log.error("executeWorkflow error: " + err);
            throw err;
        }
    },
}
