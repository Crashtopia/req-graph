    ...
    executeWorkflow: async function (opts) {
        ...
        try {
            ...
            if (hasDirectOutputToOutput(opts.flows.flows)) {
                status = "warning";
                message += "2 outputs connected to eachother.\n"
            }
            ...
            function getLLM(node) {
                if (!node.model) throw new Error("Node '" + node.name + "' has no model selected.");
                if (node.model.startsWith("gemini")) {
                    if (!llm) throw new Error("Gemini API key not set.");
                    return llm;
                }
                if (node.model.startsWith("mistral")) {
                    if (!llmMistral) throw new Error("Mistral API key not set.");
                    return llmMistral;
                }
                throw new Error("Unknown model: " + node.model);
            }

            // Invoke LLM for a node
            async function invokeNode(node, input) {
                const model = getLLM(node);
                const response = await model.invoke([
                    new SystemMessage(node.seedingPrompt || ""),
            ...
            let currentId = graph[startNode.id][0]; // first node after input
            ...
            while (currentId) {
                ...
                currentOutput = await invokeNode(node, currentOutput);
